...
class clip(TransformerMixin):
    ...
    def _check(self,data):
        ...
        assert len(self.dims) == len(self.bounds), "List of dimensions and bounds must match in length."
...
class marginalize(TransformerMixin):
    ...
    def fit(self,data):
        ...
        if self.lat_lon_weighted is True:
            assert self.lat_lon_weights is not None, "If weighted, you must provide the weights!"
            ...
            self.weight_dict = {self.weight_dims[0]: weights_d1, self.weight_dims[1]: weights_d2}
        return self
    def transform(self,data):
